...
const CompareFloatingBar = () => {
  ...
  return (
    <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 w-full max-w-md px-4 animate-in slide-in-from-bottom-10 fade-in duration-300" dir={isAr ? "rtl" : "ltr"}>
      <div className="bg-[#1a1a2e]/90 backdrop-blur-xl border border-white/10 p-4 rounded-2xl shadow-[0_0_50px_rgba(0,0,0,0.5)] flex items-center justify-between gap-4 ring-1 ring-white/5">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-xs text-gray-400">
              {selectedTools.length === 1
                ...
                : (isAr ? `${selectedTools.length} أدوات محددة` : `${selectedTools.length} selected tools`)}
            </span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <Link to="/compare">
            ...
            >
              {isAr ? "مقارنة" : "Compare"} <Arrow className={`w-4 h-4 ${isAr ? "mr-2" : "ml-2"}`} />
            </Button>
          </Link>
        </div>
      </div>
