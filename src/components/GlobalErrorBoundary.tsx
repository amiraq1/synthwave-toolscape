...
export class GlobalErrorBoundary extends Component<Props, State> {
  ...
  public static getDerivedStateFromError(error: Error): State {
    const isChunkError =
      error.name === "ChunkLoadError" || error.message.includes("dynamically imported module");
  ...
  public render() {
    ...
    if (this.state.hasError) {
      if (this.state.isChunkError) {
        this.handleReload();
        return (
          <div className="min-h-screen bg-[#0f0f1a] flex items-center justify-center text-neon-purple opacity-50 animate-pulse font-cairo" dir="rtl">
            جارٍ تحديث النظام...
          </div>
        );
      }
      ...
      return (
        <div className="min-h-screen flex flex-col items-center justify-center bg-[#0f0f1a] text-white p-4 font-cairo" dir="rtl">
          <div className="glass-card p-10 rounded-3xl flex flex-col items-center max-w-md text-center border-red-500/20 shadow-red-500/10">
            ...
            <h1 className="text-3xl font-bold mb-4">{isAr ? "عذرًا، حدث خلل غير متوقع" : "عذرًا، حدث خلل غير متوقع"}</h1>
