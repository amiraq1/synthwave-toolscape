    ...
    return (
        <Card className="bg-gradient-to-br from-indigo-900/20 to-purple-900/20 border-indigo-500/20 card-glow col-span-1 md:col-span-3 overflow-hidden relative">
            ...
            <CardHeader className="pb-2">
                <CardTitle className="flex items-center justify-between text-lg" dir={i18n.dir()}>
                    <div className="flex items-center gap-2 text-indigo-100">
                        <BarChart className="w-5 h-5 text-indigo-400" />
                        <span>{t('admin.analytics.center_title')}</span>
                    </div>
                    <Button
                        variant="ghost"
                        size="sm"
            ...
            <CardContent>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-2">
                    ...
                    {/* حالة تتبع GA4 */}
                    <div className="bg-black/40 p-4 rounded-xl border border-white/5 backdrop-blur-sm flex flex-col justify-between" dir={i18n.dir()}>
                        <div className="flex justify-between items-start mb-2">
                            <p className="text-gray-400 text-xs">{t('admin.analytics.ga4_status')}</p>
                    ...
                    <div className="bg-black/40 p-4 rounded-xl border border-white/5 backdrop-blur-sm flex flex-col justify-between" dir={i18n.dir()}>
                        <div className="flex justify-between items-start mb-2">
                            <p className="text-gray-400 text-xs">{t('admin.analytics.content_health')}</p>
                        ...
                        <div>
                            <div className="flex items-end gap-2">
                                ...
                                <span className="text-xs text-emerald-500/80 mb-1.5">{t('admin.tools.status_published')}</span>
                            </div>
                    ...
                    <div className="bg-black/40 p-4 rounded-xl border border-white/5 backdrop-blur-sm flex flex-col justify-between" dir={i18n.dir()}>
                        <div className="flex justify-between items-start mb-2">
                            <p className="text-gray-400 text-xs">{t('admin.analytics.system_status')}</p>
                            <Activity className="w-4 h-4 text-blue-400" />
                        </div>
                        <div className="space-y-2">
                            <div className="flex items-center justify-between text-xs">
                                <span className="text-gray-400">{t('admin.analytics.database')}</span>
                                <span className="text-emerald-400 flex items-center gap-1">
                                    <div className="w-1.5 h-1.5 rounded-full bg-emerald-500" /> {t('admin.analytics.connected')}
                                </span>
                            </div>
                            <div className="flex items-center justify-between text-xs">
                                <span className="text-gray-400">{t('admin.analytics.edge_functions')}</span>
                                <span className="text-emerald-400 flex items-center gap-1">
                                    <div className="w-1.5 h-1.5 rounded-full bg-emerald-500" /> {t('admin.analytics.active')}
                                </span>
                            </div>
                        </div>
                    </div>
