...
const SearchAutocomplete = ({
    ...
    const keywordSource = useMemo(
        () => (quickSuggestions && quickSuggestions.length > 0 ? quickSuggestions : DEFAULT_AR_SUGGESTIONS),
        [quickSuggestions],
    );
    ...
    const suggestions = useMemo(() => searchData?.tools ?? [], [searchData]);
    const blogPosts = useMemo(() => searchData?.posts ?? [], [searchData]);
    ...
    const handleFullSearch = useCallback(
        (e?: React.FormEvent) => {
            ...
            if (onSearch && nextQuery) onSearch(nextQuery);
            setShowSuggestions(false);
            setActiveIndex(-1);
            ...
            <span>
                {parts.map((part, i) =>
                    part.toLowerCase() === highlight.toLowerCase() ? (
                        <span key={i} className="text-neon-cyan bg-neon-cyan/10 px-0.5 rounded">
                            {part}
                        </span>
                    ) : (
                        part
