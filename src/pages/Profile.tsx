...
const Profile = () => {
  ...
  const updateProfileMutation = useMutation<ProfileUpdate, Error, ProfileUpdate, { previousProfile?: ProfileData | null }>({
    ...
    onMutate: async (newProfile) => {
      ...
      queryClient.setQueryData<ProfileData | null>(["profile", session?.user.id], (old) => (old ? { ...old, ...newProfile } : old));

      return { previousProfile };
    },
    onError: (_err, _newProfile, context) => {
      if (context?.previousProfile) queryClient.setQueryData(["profile", session?.user.id], context.previousProfile);
      toast.error(isAr ? "فشل تحديث البيانات، تمت استعادة البيانات السابقة." : "Update failed, previous data was restored.");
    },
    onSuccess: () => {
  ...
  if (authLoading || profileLoading)
    return (
      <div className="flex justify-center mt-20" role="main" dir={isAr ? "rtl" : "ltr"}>
  ...
  if (isError)
    return (
      <div className="flex flex-col items-center justify-center min-h-[50vh] text-center px-4" role="main" dir={isAr ? "rtl" : "ltr"}>
        <div className="bg-red-500/10 border border-red-500/20 rounded-2xl p-8 max-w-md">
          <h2 className="text-xl font-bold text-red-400 mb-2">{isAr ? "عذراً، حدث خطأ ما" : "Sorry, something went wrong"}</h2>
          <p className="text-gray-400 mb-6">
            {isAr ? "لم نتمكن من تحميل بيانات الملف الشخصي. حاول مرة أخرى لاحقاً." : "We could not load profile data. Please try again later."}
          </p>
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-5xl" dir={isAr ? "rtl" : "ltr"} role="main">
      <Helmet>
        <title>{t("profile.title")} | {isAr ? "نبض AI" : "Nabd AI"}</title>
      </Helmet>
      ...
      <div className="bg-white/5 border border-white/10 rounded-2xl p-8 mb-8 flex flex-col md:flex-row items-center gap-6 animate-in fade-in slide-in-from-bottom-4">
        <div className="w-24 h-24 rounded-full bg-gradient-to-tr from-neon-purple to-neon-blue p-1 relative group">
          <div className="w-full h-full rounded-full bg-black overflow-hidden flex items-center justify-center relative">
            ...
            <div
              className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-opacity cursor-pointer text-xs text-white"
        ...
        <div className={`flex-1 text-center ${isAr ? "md:text-right" : "md:text-left"} space-y-2`}>
          ...
          <div className={`flex justify-center ${isAr ? "md:justify-start" : "md:justify-start"} gap-4 mt-4 pt-2 border-t border-white/10 text-sm text-gray-500`}>
          ...
          <div className="bg-white/5 p-8 rounded-2xl border border-white/10 max-w-2xl mx-auto">
            ...
            <div className="space-y-8">
              <div className="space-y-2">
                ...
                <Input value={fullName} onChange={(e) => setFullName(e.target.value)} className="bg-black/20 border-white/10 text-white h-12" />
              </div>
              ...
              <div className="space-y-4">
                ...
                {session?.user.id && (
                  <div className="bg-black/20 p-6 rounded-xl border border-white/10">
                    <AvatarUpload userId={session.user.id} currentAvatarUrl={avatarUrl} onUploadComplete={(url) => setAvatarUrl(url)} />
                  </div>
                )}
              </div>
